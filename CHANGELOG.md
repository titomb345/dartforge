...
### Added
- Hex wilderness automapper — the Map panel is back, rebuilt from the ground up and now actually working. As you move through the wilderness, every survey paints all visible hexes (up to 19) with their terrain onto a persistent per-character map, so the world fills in around your path instead of one hex at a time. Landmarks from the survey legend (towns, caves, towers, ruins) are placed on their actual hexes and marked with a gold diamond. Rivers are detected as an overlay and drawn as a blue stream across their hex (they run through other terrain, so they'd be invisible if treated as a terrain type). Blocked movement ("You must swim...", "There is no exit...") is remembered and drawn as red edge ticks
- Click-to-walk — right-click any mapped hex to auto-walk there. Pathfinding avoids water and known blocked edges, sends one direction at a time, and verifies each step against the next survey before continuing; it stops on blocks, manual movement, or entering a building. Left-click a hex for terrain, landmarks, and visit info
