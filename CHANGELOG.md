...
- Loadout panel — a live view of what each limb is holding and everything worn, built from output the game already prints (no polling, nothing sent unless you ask). An `x me` teaches it your full state: limb roster (race-agnostic — a spyder's ten limbs work as well as a human's), held items, worn items and clothing, and per-limb health shown as colored dots. An `equip held` re-syncs just the hands. Between snapshots it follows the message stream live: hold (paired with the game's "Okay." to know exactly which hands), wear, remove, summon ("appears in your upper right hand!"), dismissal/disintegration, stow/put/take/drop, and even eating what you're carrying. Summoned items carry a ✦ badge, so a weapon that dissolves mid-hunt shows as an empty hand immediately — the panel exists to end the reflexive `eq` spam. Honesty over guessing: a bare "take" doesn't say which hand caught the item, so those land in an "in hand, limb unknown" bucket and the panel shows an "≈ approx" flag until the next sync; the ⟳ hands / ⟳ full buttons send `equip held` / `x me` for you. State persists per character and reloads flag hands as approximate (output was missed while the client was closed). Pinnable like Who/Chat/Map. Validated by replaying all 54 recorded sessions through the tracker (final states matched the known summon loadouts) plus a regression test built from real log excerpts (`scripts/test-loadout.ts`)
