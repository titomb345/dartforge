...
- `/autopowercast`: charges a focus and spends it on a powercast in a loop. It channels into the focus a set number of times with a pause between each, waits for your concentration to come back, then discharges the focus, sets your mind to isolated, runs `/powercast` with your modifier, opens your mind again and waits for your concentration before the next round of channels. Set a container and the focus can live in a pack: channelling works while it sits in there, and it is only taken out for the discharge and put straight back. Tell it the aura you have when you are topped up and it waits for that as well as your concentration before each round of channels, since channelling is what draws on your aura. Both readings come from the same trackers that feed the concentration and aura pills in the status bar, so nothing extra is sent to the game. `/autopowercast set item|container|channels|power|delay|modifier|aura <value>`, `/autopowercast on|off|status`. Only channels the MUD confirms are counted, and having no aura just waits and retries. A powercast that comes back failed (aura too weak, concentration broken, the spell fizzles) no longer stops everything: it opens your mind, waits for recovery and channels a fresh round, and only three failures in a row stop the loop. Settings are saved per character. A pink badge shows the channel count while it runs
