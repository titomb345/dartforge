...
- The Loadout panel's silent hand re-syncs no longer leak partial `equip held` output into the terminal. The gag assumed the whole response arrived in one network read and ended itself at the chunk boundary, but the response routinely splits across reads (session logs show it splitting even mid-word, especially during the 5-minute who+equip refresh burst) — so whatever limbs arrived in the next read were printed, showing anywhere from one to all of the hand lines every few minutes during long fights. The gag now survives chunk boundaries and instead ends at the first unrelated line after the response, or after a short quiet period — so a manual `eq` typed moments after a background re-sync still shows its output normally
