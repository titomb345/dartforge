...
- Door crossings now react to the MUD instead of firing blind. `/door` and the auto-walk's door handling read the server's replies as they work: keys are tried one at a time until "You unlock the …" (a "You fail." means try the next slot; "You don't have one of those." stops trying slots you don't carry; "has no lock!" skips straight to opening), the door is locked behind you with exactly the key that worked, and a door that turns out to be standing open is simply walked through. The auto-walker also remembers which key fits each mapped door — the next crossing unlocks on the first try — and stops with a clear message when a door is locked and no key on the ring fits. Unrecognized replies fall back to the old blind behavior, so nothing can hang
