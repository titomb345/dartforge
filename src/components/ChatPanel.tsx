...
export function ChatPanel({
  ...
  const identifyingMsg = identifyingMsgId != null
    ? messages.find((m) => m.id === identifyingMsgId)
    : null;
    ...
    <div className={panelRootClass(isPinned)}>
      ...
      {identifyingMsg && (
        <div className="px-2 py-1 border-b border-border-subtle shrink-0">
          <div className="text-[9px] text-text-dim mb-0.5">Identify sender for: <span className="text-text-label italic">{identifyingMsg.message}</span></div>
          <IdentifyForm
