...
> = {
  chat: {
    render: () => <ChatPanel mode="pinned" />,
    icon: (s) => <ChatIcon size={s} />,
    accent: '#8be9fd',
    label: 'Chat',
  },
  counter: {
    render: () => <CounterPanel mode="pinned" />,
    icon: (s) => <CounterIcon size={s} />,
    accent: '#f59e0b',
    label: 'Counters',
  },
  skills: {
    render: () => <SkillPanel mode="pinned" />,
    icon: (s) => <TrendingUpIcon size={s} />,
    accent: '#50fa7b',
    label: 'Skills',
  },
  notes: {
    render: () => <NotesPanel mode="pinned" />,
    icon: (s) => <NotesIcon size={s} />,
    accent: '#fbbf24',
    label: 'Notes',
  },
  map: {
  ...
  currency: {
    render: () => <CurrencyPanel mode="pinned" />,
...
export function PinnedRegion({
  ...
  return (
    <div ref={containerRef} className="flex flex-col relative" style={{ width }}>
      {panels.map((panelId, i) => {
        ...
        const style: React.CSSProperties = useFlex
          ...
          : {
              flex: 'none',
              height: `calc((100% - ${totalHandleGap}px) * ${ratios[i] ?? 1 / panels.length})`,
              minHeight: 0,
            };
        return (
          <React.Fragment key={panelId}>
            ...
            <div className="flex flex-col overflow-hidden rounded-lg bg-bg-primary" style={style}>
