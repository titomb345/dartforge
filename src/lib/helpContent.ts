  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        description:
          'Enable in Settings > Mobile Companion to start a local web server. Open the displayed URL or scan the QR code (same WiFi network) to see MUD output with colors and send commands remotely. It adapts to the device: phones get a directional d-pad and Prev/Next history buttons, while laptops/desktops get a wider layout where the numpad sends commands using your customizable DartForge numpad bindings and the arrow keys cycle history. Your character status bar — health, concentration, aura, hunger, thirst, encumbrance, movement and alignment — mirrors the desktop client just above the command input, collapsing to icon-only chips on phones (tap one to reveal its label). A sub-bar shows the in-game clock and lets you resize the output text, and your custom quick buttons (with live toggle state) appear above the input. Collapsible Who and Counters panels mirror the online-players list and your improve counters. The status chips from the desktop input row (autocast, blocked, timers) show above the quick buttons, the word next to the status dot says Live, Reconnecting, Desktop offline, or Companion off, and the phone reconnects on its own when it wakes or regains WiFi. The bell in the sub-bar turns on tell notifications, the clear button empties the output, the screen stays awake while connected, and the desktop toolbar shows a phone icon with a count while a phone is on. Add it to your home screen on iOS or Android to open it full screen.',
      },
    ],
  },
  {
