  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        description:
          "Enable in Settings > Mobile Companion to start a local web server. Open the displayed URL or scan the QR code (same WiFi network) to see MUD output with colors and send commands remotely. It adapts to the device: phones get a directional d-pad and Prev/Next history buttons, while laptops/desktops get a wider layout where the numpad sends commands using your customizable DartForge numpad bindings and the arrow keys cycle history. Your character status bar — health, concentration, aura, hunger, thirst, encumbrance, movement and alignment — mirrors the desktop client just above the command input, collapsing to icon-only chips on phones (tap one to reveal its label). A sub-bar shows the in-game clock and lets you resize the output text, and your custom quick buttons (with live toggle state) appear above the input. Collapsible Who and Counters panels mirror the online-players list and your improve counters. The word next to the status dot says Live, Reconnecting, Desktop offline, or Companion off, and the phone reconnects on its own when it wakes or regains WiFi. The bell in the sub-bar turns on tell notifications, the clear button empties the output, the screen stays awake while connected, and the desktop toolbar shows a phone icon with a count while a phone is on. Add it to your home screen on iOS or Android to open it full screen. The QR code carries the numeric address, which works in every phone browser. A second link using your computer's name (something.local) is shown beside it for Chrome on Android and Safari on iPhone; it keeps working when your address changes, but Firefox cannot open it.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        title: '/autopowercast',
        description:
          'Charge a focus, then spend it on a powercast, over and over. It channels into your focus item a set number of times with a pause between each, then waits for your concentration to come back, discharges the item, sets your mind to isolated, runs /powercast, opens your mind again and waits for your concentration to come back before the next round of channels. "/autopowercast set item <item>" picks the focus. "/autopowercast set container <name>" lets the focus live in a pack: channelling works fine while it is in there, and it is only taken out for the discharge and put straight back ("off" switches that back off). "/autopowercast set channels <n>" is how many channels to store before casting. "/autopowercast set power <n>" is the power per channel. "/autopowercast set delay <sec>" is the pause between channels. "/autopowercast set modifier <n>" is the adjustment handed to /powercast (e.g. -5). "/autopowercast set aura <level>" names the aura you have when you are topped up (e.g. "intense violet"); before each round of channels it waits for that as well as your concentration, since channelling is what draws on your aura ("off" switches that back off). It reads both from the same trackers behind the status bar pills, so nothing extra is sent to the game. "/autopowercast on" starts, "/autopowercast off" stops, "/autopowercast status" shows progress. Only channels the MUD confirms are counted; with no aura it waits and tries again. A powercast that comes back failed does not stop the loop, it recovers and channels a fresh round, but three failures in a row do stop it. Settings are remembered per character. A pink badge shows the channel count while active.',
      },
      {
        title: '/autorefresh',
        description:
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        title: 'Who List',
        description:
          'Shows online players with guild tags, soul-state dots (vibrant, engaged, distant, receptive, dim), and ANSI name colors. Auto-refreshes in the background (configurable interval in Settings > Timers). Click the refresh button for a manual update. Players using who titles (names that don\'t match the standard "Name the race" format) can be mapped to suspected or confirmed player names — hover over a title and click "?" to add a mapping. Pin the panel to keep it visible.',
        helpId: 'toolbar-who',
      },
      {
